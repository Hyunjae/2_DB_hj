...
-- 문자열을 앞에서부터 검색하여 첫 번째 B가 나오는 위치 조회
SELECT INSTR('AABAACAABBAA', 'B') FROM DUAL;

-- 문자열을 5번째부터 검색하여 첫 번째 B가 나오는 위치 조회
SELECT INSTR('AABAACAABBAA', 'B', 5) FROM DUAL;

-- 문자열을 5번째부터 검색하여 두 번째 B가 나오는 위치 조회
...
SELECT EMP_NAME, SUBSTR(EMAIL, 1, INSTR(EMAIL, '@') -1) 아이디 -- 잘라낸 부분이 남아있음
FROM EMPLOYEE; 
...
-- EMPLOYEE 테이블에서 사번이 짝수인 사원의 사번, 이름
SELECT EMP_ID , EMP_NAME
FROM EMPLOYEE
WHERE MOD(EMP_ID, 2) = 0;

-- EMPLOYEE 테이블에서 사번이 홀수인 사원의 사번, 이름
SELECT EMP_ID , EMP_NAME
FROM EMPLOYEE
WHERE MOD(EMP_ID, 2) <> 0;
...
-- ADD_MONTHS(날짜 , 숫자) : 날짜에 숫자만큼의 개월 수를 더함.(음수 가능)
...
-- EMPLOYEE 테이블에서 각 사원의 이름, 입사 년도, 월, 일 조회
...
SELECT TO_CHAR(1000000, 'L9,999,999') FROM DUAL; -- '\1,000,000' Local(원화 기호)
...
SELECT SYSDATE FROM DUAL; -- 2022-09-01 11:20:53.000

-- 2022/09/01 11:20:53 목요일
...
-- 2022년 09월 01일(목)
-- 년, 월, 일이 날짜를 나타내는 패턴으로 인식이 안되서 오류 발생하기 때문에
-- "" 쌍따옴표 이용해서 단순한 문자로 인식시키기
...
-- NVL2(컬럼명, 바꿀값1, 바꿀값2)
-- 해당 컬럼의 값이 있으면 바꿀값1로 변경(NULL이 아닐 때),
...
--> 1행이 적은 이유 : NULL값이 포함되어 있어서
-- COUNT(컬럼명)에 의해 NULL을 제외한 실제 값이 있는 행의 개수만 조회

-- EMPLOYEE 테이블에서 성별이 남성인 사원의 수
SELECT COUNT(*) FROM EMPLOYEE
WHERE SUBSTR(EMP_NO, 8, 1) = '1';

SELECT SUM(DECODE(SUBSTR(EMP_NO, 8, 1), '1', 1, 0))
FROM EMPLOYEE;  -- 값이 1이면 1을 반환, 아니면 0을 반환해서 더하기
