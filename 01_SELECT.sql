...
-- 주의사항 1. DISTINCT 구문은 SELECT마다 딱 한 번씩만 작성 가능
-- 주의사항 2. DISTINCT 구문은 SELECT 제일 앞 컬럼에 작성되어야 한다.
...
SELECT EMP_NAME, EMAIL
FROM EMPLOYEE
WHERE EMAIL LIKE '___#_%' ESCAPE '#';  -- #을 패턴이 아닌 탈출문자로 정의해줌 
--> # 뒤에 작성된 _는 일반 문자로 탈출

-- 연습문제
-- EMPLOYEE 테이블에서 이메일 '_' 앞이 4글자이면서
-- 부서 코드가 'D9' 또는 'D6'이고 -> AND가 OR보다 우선순위가 높다, () 사용 가능
