...
-- 객체를 만들고(CREATE), 바꾸고(ALTER), 삭제한다(DROP)하는 데이터 정의 언어

/* ALTER(바꾸다, 수정하다, 변조하다)
- 테이블에서 수정할 수 있는 것
1) 제약 조건(추가/삭제)  - 기존것 변경 X
2) 컬럼(추가/수정/삭제)
...
--			 [REFERENCES 테이블명[(컬럼명)]; <-- FK인 경우만 추가
...
-- ALTER TABLE 테이블명 DROP COLUMN 컬럼명; // 두 개 뜻은 동일
...
--> 컬럼이 생성 되면서 DEFAULT 값이 자동 삽입됨

-- D10 개발 1팀 추가
INSERT INTO DEPT_COPY
VALUES('D10', '개발1팀', 'L1', DEFAULT, DEFAULT);
-- ORA-12899: "KH_JHJ"."DEPT_COPY"."DEPT_ID" 열에 대한 값이 너무 큼(실제: 3, 최대값: 2)
...
ALTER TABLE DEPT_COPY RENAME TO DECOPY;
...
/* DDL 주의사항
 ...
 -> DDL은 수행 시 존재하고 있는 TRANSACTION을 모두 DB에 강제 COMMIT 시킴 
