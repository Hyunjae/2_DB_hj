...
-- 사번, 직원명, 부서코드, 부서명을 조회 하고자 할 때
SELECT EMP_ID, EMP_NAME, DEPT_CODE
FROM EMPLOYEE;
-- 사번, 직원명, 부서코드는 EMPLOYEE테이블에 조회가능
...
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);  -- ON(연결고리 역할의 컬럼)
 ...
 * -> 연결에 사용된 칼럼의 값이 일치하지 않으면 조회 결과에 포함되지 않는다!!
 */
...
-- ANSI 표준
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE RIGHT /*OUTER*/ JOIN DEPARTMENT  -- OUTER 생략 가능
...
--> 다중 조인 시 앞에서 조인된 결과의 새로운 테이블 내용을 조인
...
-- 부서가 없는 사원이지만 보너스를 받는 경우가 있기 때문에 LEFT로 고정해줌
