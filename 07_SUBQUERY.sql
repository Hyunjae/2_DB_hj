...
ORDER BY JOB_CODE;
-- SELECT절에 없는 컬럼이라도 FROM, JOIN으로 인해
-- 존재하는 컬럼이면 ORDER BY 절에 사용 가능
...
-- 1) MAIN 쿼리 1행의 DEPT_CODE를 서브쿼리에 대입
-- 2) SUB 쿼리를 수행
...
SELECT EMP_NAME, JOB_CODE, SALARY,
	...
	WHERE SUB.JOB_CODE = MAIN.JOB_CODE) "직급 급여 평균"
FROM EMPLOYEE MAIN;
...
SELECT EMP_ID, EMP_NAME, NVL(MANAGER_ID, '없음') 관리자사번, 
...
--> SELECT 해석 순서때문에
-- 급여 상위 5명이 아니라 조회순서 상위 5명의 급여 순위가 조회 됨
...
-- RANK() OVER(정렬 순위)
SELECT RANK_OVER() OVER(ORDER BY SALARY DESC) 순위,
		EMP_NAME, SALARY
FROM EMPLOYEE;
