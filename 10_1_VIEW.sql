...
--> 1) SYS 관리자 계정 접속 - 새 편집기
--  2) ALTER SESSION SET "_ORACLE_SCRIPT" = TRUE;
--  3) GRANT CREATE VIEW TO kh_jhj;
...
-- ORA-01400: NULL을 ("KH_JHJ"."DEPT_COPY2"."DEPT_TITLE") 안에 삽입할 수 없습니다
-- DEPT_TITLE에 NULL이 들어갈 수 없기 때문에 실행 불가

--> VIEW를 이용한 INSERT 시
-- VIEW가 아닌 원본 테이블에 삽입이 진행된다

-- VIEW에 포함되지 않은 컬럼(DEPT_TITLE)에는 INSERT 시 NULL이 삽입됨 ('D0', NULL, 'L3')
-- 만약 VIEW에 포함되지 않은 컬럼(DEPT_TITLE)에 NOT NULL 제약조건이 설정되어 있는 경우
