...
--									DML 수행 내용을 COMMIT, ROLLBACK하는 언어
...
-- SQL 작성법 : 대문자 작성 권장, 연결된 단어 사이에는 '_' 사용
...
-- 추가 샘플 데이터 삽입
-- 가입일 -> SYSDATE를 활용
INSERT INTO MEMBER VALUES('MEM02','ASDA1515', '김영희', '970505-2312051', SYSDATE);

-- 가입일 -> DEFAULT 활용(테이블 생성 시 정의된 값이 반영됨)
INSERT INTO MEMBER VALUES('MEM03','QWER775', '박철수', '920416-1523456', DEFAULT);

-- 가입일 -> INSERT 시 미작성 하는 경우 -> DEAFULT 값이 자동 반영됨
--INSERT INTO 테이블명(컬럼명1, 컬럼명2, 컬럼명3)
--VALUES(값1, 값2, 값3);

INSERT INTO MEMBER(MEMBER_ID, MEMBER_PWD, MEMBER_NAME)
VALUES('MEM04', 'D15F15D1', '이지연');

-- ** JDBC에서 날짜를 입력 받았을 때 삽입하는 방법 **
INSERT INTO MEMBER VALUES('MEM05', 'PASS05', '김길동', '920201-1333232', 
...
INSERT INTO MEMBER2 VALUES('MEM02', 'PASS02', '고길순', 01041045035);
--> NUMBER 타입 컬럼에 데이터 삽입 시
-- 제일 앞에 0이 있으면 자동으로 제거함
	--> 전화번호, 주민등록 번호처럼 숫자로만 되어있는 데이터지만
...
--			  NULL을 ("KH_JHJ"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
...
CREATE TABLE USER_USED_UK(
    USER_NO NUMBER,
--    USER_ID VARCHAR2(20) UNIQUE, -- 컬럼레벨 (제약조건명 미지정, 자동부여)
...
--  ORA-00001: 무결성 제약 조건(KH_JHJ.USER_ID_U)에 위배됩니다
...
-- ORA-00001: 무결성 제약 조건(KH_JHJ.USER_ID_NAME_U)에 위배됩니다
...
-- NOT NULL + UNIQUE 제약조건의 의미 -> 중복되지 않는 값이 필수로 존재해야 함(NULL X)
...
--> 기본키 중복으로 오류(무결성 제약 조건)

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가 NULL 이므로 오류
-- ORA-01400: NULL을 ("KH_JHJ"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
--> 50이라는 값은 USER_GRADE 테이블 GRADE_CODE 컬럼에서 제공하는 값이 아니므로
 ...
 -- ORA-02291: 무결성 제약조건(KH_JHJ.GRADE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다
...
-- ORA-02292: 무결성 제약조건(KH_JHJ.GRADE_CODE_FK)이 위배되었습니다- 자식 레코드가 발견되었습니다
--> 자식이 30이라는 값을 사용하고 있음
...
-->  ORA-02290: 체크 제약조건(KH_JHJ.GENDER_CHECK)이 위배되었습니다

-- CHECK 제약 조건은 범위로도 설정 가능.(BETWEEN)
...
--> 서브쿼리로 테이블 생성 시
--  테이블의 형태(컬럼명, 데이터 타입) + NOT NULL 제약조건만 복사되고
...
ALTER TABLE DEPARTMENT ADD CONSTRAINT EMP_LOCATION_ID
...
ALTER TABLE LOCATION ADD CONSTRAINT EMP_NATIONAL_CODE
