...
-- EMPLOYEE 테이블에서 직급 코드가 같은 사람의 직급 코드, 급여 평균, 인원 수를
-- 직급코드 오름 차순으로 조회
...
-- EMPLOYEE 테이블에서
-- 성별(남/여)과 각 성별 별 인원 수, 급여 합을 인원 수 내림차순으로 조회

SELECT DECODE (SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') 성별, COUNT(*) "인원 수", SUM(SALARY) "급여 합"
FROM EMPLOYEE
GROUP BY DECODE (SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') -- 별칭 사용 X(별칭 해석 X), 별칭은 SELECT절에만 사용
...
-- EMPLOYEE 테이블에서 직급 별 2000년도 이후 입사자들의 급여 합 조회
SELECT JOB_CODE, SUM(SALARY)
FROM EMPLOYEE
WHERE EXTRACT(YEAR FROM HIRE_DATE) >= 2000
-- HIRE_DATE >= '2000-01-01' -> DATE타입으로 자동 형변환
-- HIRE_DATE >= TO_DATE('2000-01-01')
-- WHERE SUBSTR(TO_CHAR(HIRE_DATE, 'YYYY'),1 ,4) >= '2000'  -> 구림
GROUP BY JOB_CODE
ORDER BY 1;
...
GROUP BY DEPT_CODE, JOB_CODE -- DEPT_CODE 로 그룹을 나누고, 
							 --나눠진 그룹 내에서 JOB_CODE 로 또 그룹을 분류한다 -> 세분화
...
FROM EMPLOYEE
--WHERE SALARY >= 3000000  --> 한 사람의 급여가 300백만 이상(요구사항 충족X)
GROUP BY DEPT_CODE
 ...
 * 
 * - UNION ALL : UNION + INTERSECT / 합집합에서 중복부분 제거 X
 * 
...
-- 조회하는 컬럼의 데이터타입, 개수가 모두 동일해야 한다!

SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
UNION 
SELECT EMP_ID, EMP_NAME, DEPT_CODE, 1   -- 데이터 타입 동일
FROM EMPLOYEE
WHERE SALARY > 3000000;
